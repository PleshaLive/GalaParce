# Файл: app.py (Версия с Log Analyzer на клиенте)
...
MAX_RAW_LOGS = 300 # Можно увеличить для анализатора
chat_messages = deque(maxlen=MAX_CHAT_MESSAGES)
raw_log_lines = deque(maxlen=MAX_RAW_LOGS)
# -------------------------

# --- Regex для парсинга чата (используется только в /submit_logs) ---
...
NAV_HTML = f"""{BASE_CSS}<nav class="navigation"><a href="/">Полный чат</a><span class="nav-separator">|</span><a href="/messages_only">Только сообщения</a><span class="nav-separator">|</span><a href="/raw_log_viewer">Анализатор Логов</a></nav>""" # Изменен текст ссылки
# ----------------------------------

# --- HTML Шаблоны для /, /messages_only (без изменений) ---
...
# --- НОВЫЙ HTML Шаблон для страницы АНАЛИЗАТОРА ЛОГОВ (/raw_log_viewer) ---
HTML_TEMPLATE_LOG_ANALYZER = """
<!DOCTYPE html>
<html lang="ru">
<head><meta charset="UTF-8"><title>CS2 Log Analyzer</title>
    <style>
        .content-wrapper h1 { margin-bottom: 20px; }
        #log-analyzer-output {
            font-family: var(--font-mono); /* Моноширинный шрифт */
            font-size: 13px;
            line-height: 1.6;
            flex-grow: 1; /* Занимает все доступное место */
            overflow-y: auto; /* Включаем скролл */
            padding: 10px;
            background-color: #181a1f; /* Темный фон для логов */
            border-radius: 6px;
        }
        .log-line {
            margin-bottom: 3px;
        ...
        /* Цвета для разных типов логов (примеры) */
        .log-line.chat { background-color: #36485e; color: #a6e3a1; border-left: 3px solid #a6e3a1;}
        .log-line.kill { background-color: #5c374f; color: #f38ba8; border-left: 3px solid #f38ba8;}
        .log-line.damage { background-color: #6e584c; color: #fab387; } /* Оранжевый */
        .log-line.grenade { background-color: #3e4b6e; color: #cba6f7; } /* Фиолетовый */
        .log-line.purchase { background-color: #2e535e; color: #89dceb; } /* Голубой */
        .log-line.pickup { background-color: #3e5a6e; color: #94e2d5; } /* Бирюзовый */
        .log-line.connect { color: #a6e3a1; } /* Зеленый текст */
        .log-line.disconnect { color: #f38ba8; } /* Красный текст */
        .log-line.system { color: var(--text-muted); font-style: italic; } /* Системные/неизвестные */
        .log-line.unknown { color: var(--text-muted); opacity: 0.8; }
    </style>
</head>
<body>
    <div class="content-wrapper">
        <h1>Анализатор Логов CS2</h1>
        <div id="log-analyzer-output">Загрузка логов...</div>
    </div>
    ...
    <script>
        const outputContainer = document.getElementById('log-analyzer-output');
        const statusText = document.getElementById('status-text');
        const loadingIndicator = document.getElementById('loading-indicator');
        let isFetching = false, errorCount = 0; const MAX_ERRORS = 5;

        // --- Регулярные выражения для JS ---
        // Примечание: Они должны быть похожи на Python Regex, но синтаксис JS немного отличается
        const chatRegex = /(\".+?\"<\d+><\[U:\d:\d+\]><\w+>)\s+(?:say|say_team)\s+\"([^\"]*)\"/i;
        const killRegex = /(\".+?\"<\d+><.+?>)\s+killed\s+(\".+?\"<\d+><.+?>)\s+with\s+\"(\w+)\"/i;
        const damageRegex = /attacked\s+\".+?\"<\d+><.+?>.*\(damage\s+\"\d+\"\)/i;
        const grenadeRegex = /threw\s+(hegrenade|flashbang|smokegrenade|molotov|decoy)/i;
        const connectRegex = /connected|entered the game/i;
        const disconnectRegex = /disconnected|left the game/i;
        const purchaseRegex = /purchased\s+\"(\w+)\"/i; // Захватываем предмет
        const pickupRegex = /picked up\s+\"(\w+)\"/i; // Захватываем предмет
        const teamSwitchRegex = /switched team to/i;
        const nameChangeRegex = /changed name to/i;
        // Можно добавить Regex для: раунд старт/конец, установка/дефьюз бомбы и т.д.

        function getLogLineInfo(line) {
            // Проверяем в порядке убывания вероятности или важности
            if (chatRegex.test(line)) return { type: 'Чат', class: 'chat' };
            if (killRegex.test(line)) return { type: 'Убийство', class: 'kill' };
            if (damageRegex.test(line)) return { type: 'Урон', class: 'damage' };
            if (grenadeRegex.test(line)) return { type: 'Граната', class: 'grenade' };
            if (purchaseRegex.test(line)) return { type: 'Покупка', class: 'purchase' };
            if (pickupRegex.test(line)) return { type: 'Подбор', class: 'pickup' };
            if (connectRegex.test(line)) return { type: 'Подключение', class: 'connect' };
            if (disconnectRegex.test(line)) return { type: 'Отключение', class: 'disconnect' };
            if (teamSwitchRegex.test(line)) return { type: 'Смена команды', class: 'system' };
            if (nameChangeRegex.test(line)) return { type: 'Смена ника', class: 'system' };
            // Добавьте другие проверки здесь...

            // Если ничего не подошло
            return { type: 'Неизвестно/Система', class: 'unknown' };
        }
        ...
        async function fetchAndAnalyzeLogs() {
            ...
            try {
                const response = await fetch('/raw_json'); // Запрашиваем СЫРЫЕ логи
                if (!response.ok) throw new Error(`Ошибка сети: ${response.status}`);
                const logLines = await response.json(); // Массив строк лога
                const isScrolledToBottom = outputContainer.scrollTop + outputContainer.clientHeight >= outputContainer.scrollHeight - 50;

                outputContainer.innerHTML = ''; // Очищаем контейнер

                if (logLines.length === 0) {
                    outputContainer.textContent = 'Нет данных лога для анализа.';
                } else {
                    logLines.forEach(line => {
                        const info = getLogLineInfo(line); // Определяем тип строки
                        const lineDiv = document.createElement('div');
                        lineDiv.className = `log-line ${info.class}`; // Присваиваем класс для стилизации
                        lineDiv.textContent = line; // Показываем саму строку
                        lineDiv.title = `Тип: ${info.type}`; // Всплывающая подсказка с типом
                        outputContainer.appendChild(lineDiv);
                    });
                }

                if (isScrolledToBottom) {
                    setTimeout(() => { outputContainer.scrollTop = outputContainer.scrollHeight; }, 0);
                 }
                statusText.textContent = `Обновлено: ${new Date().toLocaleTimeString()} (${logLines.length} строк)`; errorCount = 0;
            } catch (error) {
                 outputContainer.textContent = 'Ошибка загрузки или анализа логов.'; console.error(error);
        ...
        const intervalId = setInterval(fetchAndAnalyzeLogs, 5000); // Обновляем реже
        fetchAndAnalyzeLogs();
    </script>
</body>
</html>
...
# Эндпоинт для НОВОЙ страницы АНАЛИЗАТОРА ЛОГОВ (/raw_log_viewer)
@app.route('/raw_log_viewer', methods=['GET'])
def raw_log_viewer_page():
    full_html = BASE_CSS + NAV_HTML + HTML_TEMPLATE_LOG_ANALYZER # Используем новый шаблон
    return Response(full_html, mimetype='text/html')
