# Файл: app.py (Версия с исправленным Regex для чата)
...
# ИСПРАВЛЕНО: Улучшен Regex для поддержки формата даты/времени и сообщений без кавычек
CHAT_REGEX_SAY = re.compile(
    r"""
    ^\s* # Начало строки
    (?:\d{2}\/\d{2}\/\d{4}\s+-\s+)? # Опциональная Дата ДД/ММ/ГГГГ -
    (?P<timestamp>\d{2}:\d{2}:\d{2}\.\d{3}) # Захват времени ЧЧ:ММ:СС.мс
    \s+-\s+ # Разделитель ' - '
    (?P<player_info>\".+?\"<\d+><\[U:\d:\d+\]><\w+>) # Информация об игроке (как и раньше)
    \s+
    (?:say|say_team) # Слово 'say' или 'say_team'
    \s+
    (?P<message>.*) # Захватываем ВСЁ остальное до конца строки как сообщение
    \s*$ # Конец строки
    """,
...
# (Они остаются без изменений, т.к. правки были только в Regex)
...
def receive_and_parse_logs_handler():
    ...
    for line in log_lines:
        ...
        # Используем обновленный, более точный Regex
        ...
        if match:
            ...
            # Захватываем сообщение ИЗ Regex группы 'message'
            message = html.escape(extracted_data['message'].strip())
            # Захватываем время ИЗ Regex группы 'timestamp'
            timestamp = extracted_data.get('timestamp', current_time.strftime('%H:%M:%S.%f')[:-3]) # Fallback на текущее время
    ...
    # Добавляем только найденные сообщения чата в очередь chat_messages
...
def raw_log_viewer_page():
    full_html = BASE_CSS + NAV_HTML + HTML_TEMPLATE_LOG_ANALYZER
    return Response(full_html, mimetype='text/html')
