...
MAX_RAW_LOGS = 300
chat_messages = deque(maxlen=MAX_CHAT_MESSAGES) # Будет хранить объекты сообщений с информацией о команде
raw_log_lines = deque(maxlen=MAX_RAW_LOGS)
current_scoreboard_data = {"fields": [], "players": []}
# -----------------------

# --- Regex Definitions ---
# MODIFIED: CHAT_REGEX_SAY для захвата имени, команды игрока и типа команды (say/say_team)
...
# MODIFIED: Добавлены CSS переменные для цветов команд в чате
BASE_CSS = """<style>
:root{
    --bg-color:#1a1d24;
    --container-bg:#232730;
    --container-border:#3b4048;
    --text-color:#cdd6f4;
    --text-muted:#a6adc8;
    --accent-color-1:#89b4fa; /* Основной акцентный */
    --accent-color-2:#a6e3a1;
    --link-color:var(--accent-color-2);
    --link-hover-bg:#3e4451;
    --error-color:#f38ba8;
    ...
    /* NEW: CSS переменные для цветов команд в чате */
    --chat-team-ct-color: #89b4fa; /* Синий (совпадает с accent-color-1) */
    --chat-team-t-color: #FFD700;  /* Желтый/золотой */
    --chat-sender-default-color: var(--accent-color-1); /* Цвет для ников без спец. команды или Spectator */
}
...
::-webkit-scrollbar-thumb:hover{background:#6E738D;}
...
# --- HTML Templates ---
# MODIFIED: Стили для .sender и его вариаций теперь используют CSS переменные
...
<style>
    ...
    /* MODIFIED: Стили для .sender и его вариаций теперь используют CSS переменные из BASE_CSS */
    .message .sender{
        font-weight:600;
        margin-right:5px;
        color: var(--chat-sender-default-color); /* Цвет по умолчанию из CSS переменной */
    }
    .message .sender.team-ct {
        color: var(--chat-team-ct-color); /* Цвет для CT из CSS переменной */
    }
    .message .sender.team-t {
        color: var(--chat-team-t-color); /* Цвет для T из CSS переменной */
    }
    .message .text{}
...
async function fetchMessages(){
    ...
    isFetching=!0;loadingIndicator.style.display='inline-block';
    try{
        const response=await fetch('/chat');
        if(!response.ok)throw new Error('Ошибка сети: '+response.status);
        const messages=await response.json(); // Ожидаем, что messages - это массив объектов с полями ts, sender, msg, team
        ...
        if(messages.length===0){
            ...
            messages.forEach(data=>{
                ...
                senderSpan.className='sender'; // Базовый класс
                senderSpan.textContent=data.sender+':';
                
                // NEW: Применение CSS класса в зависимости от команды игрока
                if (data.team === 'CT') {
                    senderSpan.classList.add('team-ct');
                } else if (data.team === 'T') {
                    senderSpan.classList.add('team-t');
                }
                // Если data.team не 'CT' и не 'T', будет использован стиль по умолчанию для .sender
        ...
        if(isScrolledToBottom){setTimeout(()=>{chatContainer.scrollTop=chatContainer.scrollHeight},0)}
        statusText.textContent='Обновлено: '+new Date().toLocaleTimeString();errorCount=0
    }catch(error){
        console.error('Ошибка:',error);statusText.textContent='Ошибка: '+error.message+'. #'+(errorCount+1);errorCount++;
        if(errorCount>=MAX_ERRORS){statusText.textContent+=' Автообновление остановлено.';clearInterval(intervalId)}
    }finally{
        isFetching=!1;loadingIndicator.style.display='none'}
    }
...
def receive_and_parse_logs_handler():
    ...
    if not log_lines: return jsonify({"status": "error", "message": "Строки не предоставлены или не удалось их извлечь"}), 400
    ...
    for line_content in log_lines:
        ...
        if not is_capturing_json_block and "JSON_BEGIN{" in stripped_content_for_markers:
            ...
            if "}}JSON_END" in temp_json_lines_buffer[0]:
                ...
                is_capturing_json_block = False; logger.debug("Однострочный JSON блок будет обработан.")
            else: continue
            ...
            if "}}JSON_END" in stripped_content_for_markers:
                ...
                if end_marker_pos != -1: temp_json_lines_buffer[-1] = temp_json_lines_buffer[-1][:end_marker_pos + 2]
        ...
        if not is_capturing_json_block:
            ...
            if chat_match:
                ...
                # NEW: Фильтруем: показываем только 'say', а не 'say_team'
                if chat_command == "say":
                    sender_name = html.escape(extracted_data['player_name'].strip())
                    message_text = html.escape(extracted_data['message'].strip())
                    timestamp_str = extracted_data.get('timestamp', datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3])
                    player_team_raw = extracted_data['player_team'] # e.g., "CT", "TERRORIST", "Spectator"

                    if not message_text: # Пропускаем пустые сообщения
                        continue

                    # NEW: Определение идентификатора команды для цветового кодирования
                    team_identifier = "Other" # По умолчанию
                    if player_team_raw.upper() == "CT": # Сравнение без учета регистра
                        team_identifier = "CT"
                    ...
                    message_obj = {
                        ...
                        "sender": sender_name,
                        "msg": message_text,
                        "team": team_identifier # Добавляем команду в объект сообщения
                    }
                    parsed_chat_batch.append(message_obj)
                    new_chat_messages_count += 1
                # Сообщения 'say_team' просто игнорируются и не добавляются в parsed_chat_batch
    
    if parsed_chat_batch:
        chat_messages.extend(parsed_chat_batch)
        if new_chat_messages_count > 0:
            logger.info(f"Log Parser: Добавлено {new_chat_messages_count} сообщений ОБЩЕГО чата.") # Уточнено
...
def get_structured_chat_data():
    return jsonify(list(chat_messages)) # Теперь chat_messages содержит только 'say' сообщения с полем 'team'
...
def messages_only_page():
    # Этот эндпоинт не изменялся и будет показывать только текст сообщений 'say'
    # так как /chat теперь отдает только их.
