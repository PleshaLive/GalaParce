...
<nav class="navigation">
    ...
    <span class="nav-separator">|</span>
    <a href="/raw_log_viewer">Показать Логи</a>
</nav>
"""
# ----------------------------------
            ...
            try {
                ...
                if (messages.length === 0) {
                    ...
                    messages.forEach(data => {
                        ...
                        const timeSpan = document.createElement('span'); timeSpan.className = 'timestamp'; timeSpan.textContent = `[${data.ts}]`;
                        const senderSpan = document.createElement('span'); senderSpan.className = 'sender'; senderSpan.textContent = data.sender + ':';
                        const textSpan = document.createElement('span'); textSpan.className = 'text'; textSpan.textContent = data.msg;
                        messageElement.appendChild(timeSpan); messageElement.appendChild(senderSpan); messageElement.appendChild(textSpan);
...
<head>
    ...
    <style>
        #messages-list {font-family: var(--font-primary);font-size: 1.05em;line-height: 1.7;padding: 15px;}
        #messages-list div {margin-bottom: 8px;padding-left: 10px;border-left: 3px solid var(--accent-color-1);}
        .content-wrapper h1 { margin-bottom: 20px; }
    </style>
</head>
<body>
    <div class="content-wrapper">
        <h1>Только сообщения</h1>
...
<head><meta charset="UTF-8"><title>CS2 Raw Logs</title>
    <style>
        pre {font-family: var(--font-mono);font-size: 13px;line-height: 1.5;white-space: pre-wrap;word-wrap: break-word;background-color: #181a1f;padding: 15px;border-radius: 6px;border: 1px solid var(--container-border);flex-grow: 1;overflow-y: auto;}
        .content-wrapper h1 { margin-bottom: 20px; }
    </style>
</head>
<body>
    ...
    <script>
        const preElement=document.getElementById('log-data');const statusText=document.getElementById('status-text');const loadingIndicator=document.getElementById('loading-indicator');let isFetching=!1,errorCount=0;const MAX_ERRORS=5;
        async function fetchRawLogs(){if(isFetching||errorCount>=MAX_ERRORS)return;isFetching=!0;loadingIndicator.style.display='inline-block';try{const response=await fetch('/raw_json');if(!response.ok)throw new Error('Network error');const data=await response.json();const isScrolledToBottom=preElement.scrollTop+preElement.clientHeight>=preElement.scrollHeight-30;preElement.textContent=data.join('\\n');if(isScrolledToBottom){setTimeout(()=>{{preElement.scrollTop=preElement.scrollHeight}},0)}statusText.textContent=`Обновлено: ${new Date().toLocaleTimeString()}`;errorCount=0}catch(error){preElement.textContent='Ошибка загрузки сырых логов.';console.error(error);statusText.textContent=`Ошибка: ${error.message}. #${errorCount+1}`;errorCount++;if(errorCount>=MAX_ERRORS){statusText.textContent+=' Автообновление остановлено.';clearInterval(intervalId)}}finally{isFetching=!1;loadingIndicator.style.display='none'}}
        const intervalId=setInterval(fetchRawLogs,5000);fetchRawLogs();
    </script>
</body>
</html>
"""
