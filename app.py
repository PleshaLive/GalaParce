...
# --- Regex Definitions ---
# Regex to capture chat messages (say and say_team)
...
# Regex to capture the scoreboard field headers
...
# Regex to capture individual player data lines for the scoreboard
...
# (These sections remain unchanged as they don't affect scoreboard order)
...
# --- HTML Templates ---
# (HTML_TEMPLATE_MAIN, HTML_TEMPLATE_MSG_ONLY, HTML_TEMPLATE_LOG_ANALYZER remain unchanged)
...
// Затем отображаем Террористов (T)
...
@app.route('/gsi', methods=['POST']) # Assuming /gsi might also send similar log formats
def receive_and_parse_logs_handler():
    """
    Receives log lines (either as JSON array or raw text lines),
    parses them for chat messages and scoreboard data, and stores them.
    """
    global chat_messages, raw_log_lines, current_scoreboard_data
    log_lines = []

    # Determine input format (JSON or raw text)
    if request.is_json:
        data = request.get_json()
        if isinstance(data, dict) and 'lines' in data and isinstance(data.get('lines'), list):
            log_lines = data.get('lines', [])
        else:
            app.logger.warning("Received JSON, but 'lines' key is missing or not a list.")
            # Optionally try to handle other JSON structures if needed
    else:
    ...
    if not log_lines:
        app.logger.warning("Received request but no log lines found.")
        return jsonify({"status": "error", "message": "No lines provided or invalid format"}), 400
    ...
    temp_scoreboard_players = [] # Temporarily store player data for the current batch
    ...
    for line in log_lines:
        ...
        if chat_match:
            ...
            # Extract only the name part before the tags like 
            name_match = re.search(r'^([^\<]+)', player_name_and_tags_str)
            sender = html.escape(name_match.group(1).strip()) if name_match else html.escape(player_name_and_tags_str.strip()) # Fallback to full string if regex fails
            message = html.escape(extracted_data['message'].strip())
            timestamp = extracted_data.get('timestamp', 'N/A') # Get timestamp if available
        ...
        # --- Try parsing as Scoreboard Fields ---
        ...
        if fields_match:
            ...
            if new_fields:
                # Only update if fields actually changed or were empty before
                if new_fields != current_scoreboard_data.get('fields'):
                    current_scoreboard_data['fields'] = new_fields
                    current_scoreboard_data['players'] = [] # Reset players when fields change
                    temp_scoreboard_players = [] # Also clear the temporary batch
                    new_scoreboard_data_parsed = True
                    app.logger.info(f"Scoreboard: Updated fields: {current_scoreboard_data['fields']}")
            else:
                 app.logger.warning(f"Scoreboard: Received empty fields line: {line}")
            continue # Move to next line

        # --- Try parsing as Scoreboard Player Data ---
        player_match = SCOREBOARD_PLAYER_REGEX.search(line)
        # Ensure fields are defined before processing player data
        if player_match and current_scoreboard_data.get('fields'):
            ...
            if len(player_values) == len(current_scoreboard_data['fields']):
                ...
                new_scoreboard_data_parsed = True # Mark that we parsed player data in this batch
            else:
                app.logger.warning(
                    f"Scoreboard: Mismatch between player values ({len(player_values)}) and fields ({len(current_scoreboard_data['fields'])}). "
                    f"Fields: {current_scoreboard_data['fields']}, Values: {player_values}. Line: {line}"
                )
            continue # Move to next line

        # --- Add other log parsing logic here if needed ---

    # Update global stores after processing the batch
    if parsed_chat_batch:
        chat_messages.extend(parsed_chat_batch)
        if new_chat_messages_count > 0:
             app.logger.info(f"Log Parser: Added {new_chat_messages_count} chat messages.")

    # If we parsed new player data in this batch, add it to the main player list
    if temp_scoreboard_players:
         # Note: This currently appends. If a full refresh is intended per batch,
         # you might want `current_scoreboard_data['players'] = temp_scoreboard_players`
         # but this depends on how the logs are generated (full snapshot vs individual lines).
         # Based on the fields logic, seems like a full reset happens on 'fields' line.
         current_scoreboard_data['players'].extend(temp_scoreboard_players)
         if new_scoreboard_data_parsed: # Log only if we actually added players in *this* run
             app.logger.info(f"Scoreboard: Added/updated {len(temp_scoreboard_players)} players.")

    return jsonify({
        "status": "success",
        "message": f"Processed {len(log_lines)} lines. Added {new_chat_messages_count} chat messages. Scoreboard parsed: {new_scoreboard_data_parsed}"
    }), 200

@app.route('/chat', methods=['GET'])
def get_structured_chat_data():
    """Returns the stored chat messages as a JSON list."""
    return jsonify(list(chat_messages))

@app.route('/raw_json', methods=['GET'])
def get_raw_log_lines():
    """Returns the stored raw log lines as a JSON list."""
    return jsonify(list(raw_log_lines))

@app.route('/scoreboard_json', methods=['GET'])
def get_scoreboard_data():
    """Returns the current scoreboard data (fields and players) as JSON."""
    return jsonify(current_scoreboard_data)

# --- HTML Page Routes ---
@app.route('/', methods=['GET'])
def index():
    """Serves the main page with full chat view."""
    html_content = BASE_CSS + NAV_HTML + HTML_TEMPLATE_MAIN
    return Response(html_content, mimetype='text/html')

@app.route('/messages_only', methods=['GET'])
def messages_only_page():
    """Serves the page showing only chat message content."""
    html_content = BASE_CSS + NAV_HTML + HTML_TEMPLATE_MSG_ONLY
    return Response(html_content, mimetype='text/html')

@app.route('/raw_log_viewer', methods=['GET'])
def raw_log_viewer_page():
    """Serves the raw log viewer/analyzer page."""
...
def scoreboard_viewer_page():
    """Serves the scoreboard viewer page."""
    # Используем ИЗМЕНЕННЫЙ шаблон HTML_TEMPLATE_SCOREBOARD
