...
const stopStreamBtn = document.getElementById('stopStreamBtn'); // Новая кнопка
const statusEl = document.getElementById('status');
const setupControlsEl = document.getElementById('setupControls'); // Контейнер с селектами

let localStream;
let myUniqueWebcamId = 'player-' + Math.random().toString(36).substring(2, 9);
let peerConnections = {}; 
let currentRegisteredNickname = null; // Для отслеживания, под каким ником зарегистрирован
...
socket.on('connect', () => {
    ...
    requestPlayerList(); // Запрашиваем при подключении
});
...
socket.on('player_setup_data_available', (serverPlayers) => {
    ...
    const currentSelectedValue = nicknameSelect.value; // Сохраняем текущий выбор, если есть
    ...
    if (serverPlayers && serverPlayers.length > 0) {
        serverPlayers.forEach(player => {
            ...
            if (player.isRegistered) {
                ...
                option.classList.add('registered-player'); // Добавляем класс для CSS
            }
...
async function populateWebcamList() { /* ... код как в #37 ... */ console.log('PlayerJS: populateWebcamList'); try { const devices = await navigator.mediaDevices.enumerateDevices(); const videoDevices = devices.filter(device => device.kind === 'videoinput'); webcamSelect.innerHTML = videoDevices.map(d => `<option value="${d.deviceId}">${d.label||`Камера ${webcamSelect.options.length+1}`}</option>`).join(''); if (videoDevices.length > 0) await startPreview(videoDevices[0].deviceId); else { statusEl.textContent = 'Веб-камеры не найдены.'; checkStartButtonState(); } } catch (e) { statusEl.textContent = 'Ошибка доступа к устройствам: '+e.message; console.error('PlayerJS: Enum devices error:', e); checkStartButtonState(); } }
async function startPreview(deviceId) { /* ... код как в #37 ... */ console.log(`PlayerJS: startPreview for deviceId: ${deviceId}`); if (localStream) { localStream.getTracks().forEach(track => track.stop()); } const constraints = { video: { deviceId: deviceId ? { exact: deviceId } : undefined, width: {ideal:640}, height: {ideal:480} }, audio: true }; try { localStream = await navigator.mediaDevices.getUserMedia(constraints); localVideo.srcObject = localStream; statusEl.textContent = 'Предпросмотр камеры активен.'; } catch (e) { statusEl.textContent = 'Ошибка доступа к веб-камере: '+e.message; console.error('PlayerJS: getUserMedia error:',e); localStream = null; } checkStartButtonState(); }
webcamSelect.onchange = () => { /* ... код как в #37 ... */ console.log('PlayerJS: Webcam changed:', webcamSelect.value); startPreview(webcamSelect.value); };
...
startStreamBtn.onclick = () => {
    ...
    currentRegisteredNickname = selectedOption.dataset.name; // Сохраняем ник
...
stopStreamBtn.onclick = () => {
    ...
    checkStartButtonState(); // Обновить состояние кнопки Start
    requestPlayerList(); // Обновить список игроков, т.к. наш слот освободился
};


socket.on('registration_success', (data) => { /* ... код как в #37 ... */ console.log('PlayerJS: Reg success:', data); statusEl.textContent = `Вы транслируете как ${data.nickname}. Ожидание зрителей...`; setupControlsEl.querySelectorAll('select, input').forEach(el => el.disabled = true); startStreamBtn.style.display = 'none'; stopStreamBtn.style.display = 'inline-block'; });
socket.on('registration_error', (message) => { /* ... код как в #37 ... */
...
socket.on('unregistration_success', () => {
    ...
    // UI уже должен быть обновлен локально в stopStreamBtn.onclick
});


// --- WebRTC Logic (без изменений с предыдущего полного ответа #37) ---
socket.on('webrtc_offer_from_viewer', async ({ offer, viewerWebcamId }) => { /* ... код как в #37 ... */ });
socket.on('webrtc_ice_candidate_from_peer', async ({ candidate, iceSenderId }) => { /* ... код как в #37 ... */ });
// Полный WebRTC код из #37:
